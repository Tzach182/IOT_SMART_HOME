...
DHT_topic = 'healthUpdate/person/id/status'
...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self, mc):
        ...
        self.bodyTemperature = QLineEdit()
        self.bodyTemperature.setText('')

        self.pulseRate = QLineEdit()
        self.pulseRate.setText('')

        self.respirationRate = QLineEdit()
        self.respirationRate.setText('')

        self.bloodPressure = QLineEdit()
        self.bloodPressure.setText('')
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # set up main window
        self.setGeometry(30, 600, 600, 150)
        self.setWindowTitle('Body Monitor')
    ...
    def update_data(self):
        print('Next update')
        bodyTemp = 36 + random.randrange(1, 10) / 10
        pulseRate = 70 + random.randrange(1, 25) / 10
        respirationRate = 15 + random.randrange(1, 10) / 10
        bloodPressure = 79 + random.randrange(1, 25) / 10
        dict = {"type" : "1", "bodyTemperature": str(bodyTemp), "pulseRate": str(pulseRate), "respirationRate": str(respirationRate), "bloodPressure": str(bloodPressure)}
        current_data = json.dumps(dict)
        self.connectionDock.bodyTemperature.setText(str(bodyTemp))
        self.connectionDock.pulseRate.setText(str(pulseRate))
        self.connectionDock.respirationRate.setText(str(respirationRate))
        self.connectionDock.bloodPressure.setText(str(bloodPressure))
